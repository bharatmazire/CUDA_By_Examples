...
#define CHECK_IMAGE_WIDTH 4096
#define CHECK_IMAGE_HEIGHT 4096
...
struct cuComplexCPU
{
	float r;
	float i;
	cuComplexCPU(float a, float b) : r(a), i(b)
	{
	}
	...
	cuComplexCPU operator*(const cuComplexCPU& a)
	{
		return cuComplexCPU(r * a.r - i * a.i, i * a.r + r * a.i);
	}
	cuComplexCPU operator+(const cuComplexCPU& a)
	{
		return cuComplexCPU(r + a.r, i + a.i);
	}
};
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	MSG			msg;
	WNDCLASSEX	wndclass;
	TCHAR		szAppName[] = TEXT("Julia Set - CPU");
	HWND		hwnd;
...
void loadTexture(void)
{
	...
	void MakeCheckImageCPU(void);
	void InitializeGPU(int, unsigned char*);

	// code

	//MakeCheckImageCPU();				// call this for CPU
...
void MakeCheckImageCPU(void)
{
	...
	for (i = 0; i < CHECK_IMAGE_HEIGHT; i++)
	{
		...
		{
			int offset = i + j * CHECK_IMAGE_WIDTH;			// as CHECK_IMAGE_WIDTH and CHECK_IMAGE_HEIGHT both same
			int juliaValue = juliaCPU(i, j);
			/* // old method by using 3-D array
			CheckImage[i][j][0] = (GLubyte)255 * juliaValue;
			CheckImage[i][j][1] = (GLubyte)0;
			CheckImage[i][j][2] = (GLubyte)0;
			CheckImage[i][j][3] = (GLubyte)255;
			*/
			CheckImage[offset * 4 + 0] = (GLubyte)255 * juliaValue;
			CheckImage[offset * 4 + 1] = (GLubyte)0;
			CheckImage[offset * 4 + 2] = (GLubyte)0;
			CheckImage[offset * 4 + 3] = (GLubyte)255;
		}
	}
}
...
int juliaCPU(int x, int y)
{
	...
	float jx = scale * (float)(CHECK_IMAGE_WIDTH / 2 - x) / (CHECK_IMAGE_WIDTH / 2);		// as CHECK_IMAGE_WIDTH and CHECK_IMAGE_HEIGHT both same
	float jy = scale * (float)(CHECK_IMAGE_WIDTH / 2 - y) / (CHECK_IMAGE_WIDTH / 2);		// as CHECK_IMAGE_WIDTH and CHECK_IMAGE_HEIGHT both same

	cuComplexCPU c(-0.8f, 0.156f);
	cuComplexCPU a(jx, jy);
