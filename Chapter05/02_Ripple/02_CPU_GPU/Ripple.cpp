...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	MSG			msg;
	WNDCLASSEX	wndclass;
	TCHAR		szAppName[] = TEXT("CUDA Ripples");
	HWND		hwnd;
...
void InitializeCPU(void)
{
	...
	for (i = 0; i < CHECK_IMAGE_HEIGHT; i++)
	{
		for (j = 0; j < CHECK_IMAGE_WIDTH; j++)
		{
			...
			int offset = i + j * 16 * CHECK_IMAGE_WIDTH/16;

			float fx = i - CHECK_IMAGE_WIDTH / 2;
			float fy = j - CHECK_IMAGE_WIDTH / 2;
			
			float d = sqrtf(fx * fx + fy * fy);
			
			GLubyte grey = (GLubyte)(128.0f + 127.0f * cos(d / 10.0f - ticks / 7.0f) / (d / 10.0f + 1.0f));

			//c = (((i & 0x8) == 0) ^ ((j & 0x8) == 0)) * 255;

			CheckImage[offset * 4 + 0] = (GLubyte)0;
			CheckImage[offset * 4 + 1] = (GLubyte)grey;
			CheckImage[offset * 4 + 2] = (GLubyte)grey;
